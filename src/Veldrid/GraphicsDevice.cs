...
namespace Veldrid
{
    ...
    public abstract class GraphicsDevice : IDisposable
    {
        ...
        public virtual bool SyncToVerticalBlank
        {
            ...
            set
            {
                if (MainSwapchain == null) throw new VeldridException("This GraphicsDevice was created without a main Swapchain. This property cannot be set.");
        ...
        /// </param>
        /// <param name="fence">
        ///     A <see cref="Fence" /> which will become signaled after this submission fully completes
        ///     execution.
        /// </param>
        ...
        {
            if (!WaitForFence(fence, ulong.MaxValue)) throw new VeldridException("The operation timed out before the Fence was signaled.");
        }
        ...
        {
            return WaitForFence(fence, (ulong)timeout.TotalMilliseconds * 1_000_000);
        }
        ...
        /// <param name="waitAll">
        ///     If true, then this method blocks until all of the given Fences become signaled.
        ///     If false, then this method only waits until one of the Fences become signaled.
        /// </param>
        public void WaitForFences(Fence[] fences, bool waitAll)
        {
            if (!WaitForFences(fences, waitAll, ulong.MaxValue)) throw new VeldridException("The operation timed out before the Fence(s) were signaled.");
        }
        ...
        {
            return WaitForFences(fences, waitAll, (ulong)timeout.TotalMilliseconds * 1_000_000);
        }
        ...
        ///     A value in nanoseconds, indicating the maximum time to wait on the Fence.  Pass
        ///     ulong.MaxValue to wait indefinitely.
        /// </param>
        /// <returns>True if the Fence was signaled. False if the timeout was reached instead.</returns>
        ...
        {
            if (MainSwapchain == null) throw new VeldridException("This GraphicsDevice was created without a main Swapchain, so the requested operation cannot be performed.");
        ...
        /// <summary>
        ///     Notifies this instance that the main window has been resized. This causes the <see cref="SwapchainFramebuffer" />
        ///     to
        ...
        /// <returns>
        ///     A <see cref="TextureSampleCount" /> value representing the maximum count that a <see cref="Texture" /> of that
        ///     format can be created with.
        /// </returns>
        public abstract TextureSampleCount GetSampleCountLimit(PixelFormat format, bool depthFormat);

        /// <summary>
        ///     Maps a <see cref="DeviceBuffer" /> or <see cref="Texture" /> into a CPU-accessible data region. For Texture
        ///     resources, this
        ...
        public MappedResource Map(IMappableResource resource, MapMode mode, uint subresource)
        {
            ...
            if (resource is DeviceBuffer buffer)
            {
                ...
                if (subresource != 0) throw new VeldridException("Subresource must be 0 for Buffer resources.");
            ...
            {
                if ((tex.Usage & TextureUsage.Staging) == 0) throw new VeldridException("Texture must have the Staging usage flag to be mapped.");
        ...
        /// <param name="source">
        ///     A pointer to the start of the data to upload. This must point to tightly-packed pixel data for
        ///     the region specified.
        /// </param>
        /// <param name="sizeInBytes">
        ///     The number of bytes to upload. This value must match the total size of the texture region
        ///     specified.
        /// </param>
        ...
        {
            fixed (void* pin = &MemoryMarshal.GetReference(source)) UpdateBuffer(buffer, bufferOffsetInBytes, (IntPtr)pin, (uint)(sizeof(T) * source.Length));
        }
        ...
        /// <summary>
        ///     Gets whether or not the given <see cref="PixelFormat" />, <see cref="TextureType" />, and
        ///     <see cref="TextureUsage" />
        ...
        ///     If the combination is supported, then this parameter describes the limits of a Texture
        ///     created using the given combination of attributes.
        /// </param>
        /// <returns>
        ///     True if the given combination is supported; false otherwise. If the combination is supported,
        ///     then <paramref name="properties" /> contains the limits supported by this instance.
        /// </returns>
        public bool GetPixelFormatSupport(
            PixelFormat format,
            TextureType type,
            ...
            uint mipLevel, uint arrayLayer)
        {
            if (FormatHelpers.IsCompressedFormat(texture.Format))
            {
                if (x % 4 != 0 || y % 4 != 0 || height % 4 != 0 || width % 4 != 0)
                {
                    ...
                    if (width != mipWidth && height != mipHeight) throw new VeldridException("Updates to block-compressed textures must use a region that is block-size aligned and sized.");
                }
            }
            ...
            if (x + width > roundedTextureWidth || y + height > roundedTextureHeight || z + depth > texture.Depth) throw new VeldridException("The given region does not fit into the Texture.");
            ...
            if ((texture.Usage & TextureUsage.Cubemap) != 0) effectiveArrayLayers *= 6;
...
#if !EXCLUDE_VULKAN_BACKEND
        /// <summary>
        ///     Tries to get a <see cref="BackendInfoVulkan" /> for this instance. This method will only succeed if this is a
        ///     Vulkan
        ///     GraphicsDevice.
        /// </summary>
        ...
        {
            if (!GetVulkanInfo(out var info)) throw new VeldridException($"{nameof(GetVulkanInfo)} can only be used on a Vulkan GraphicsDevice.");
        ...
        {
            if (!GetMetalInfo(out var info)) throw new VeldridException($"{nameof(GetMetalInfo)} can only be used on a Metal GraphicsDevice.");
        ...
        /// </summary>
        /// <param name="x"></param>
        /// x coordinate of the display's origin
        /// <param name="y"></param>
        /// y coordinate of the display's origin
        /// <param name="w"></param>
        /// display's width
        /// <param name="h"></param>
        /// display's height
...
// path is deferred to S6 if a real consumer needs it.
        ...
        /// <param name="options">Describes several common properties of the GraphicsDevice.</param>
        /// <param name="nsWindow">
        ///     A pointer to an NSWindow object, which will be used to create the Metal device's swapchain.
        /// </param>
        /// <returns>A new <see cref="GraphicsDevice" /> using the Metal API.</returns>
